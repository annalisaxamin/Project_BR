...
To perform the DEG analysis, we first need to create a DGRList object containing information about counts, annotation, samples and genes. To do that, we use the function `DGEList` to create the input for the following normalization step. The normalization intra- and inter-sample is done using the function `calcNormFactors` and specifying `method = "TMM"`, which is the weighted trimmed mean of M-values approach. 
...
We define the experimental design matrix, later needed to calculate the dispersion and fit.
```{r}
...
- unchanged regulated genes otherwise.
...
ego_BP_up <- enrichGO(gene = upDEGs$external_gene_name.y,
                   OrgDb = org.Hs.eg.db,
                   keyType = 'SYMBOL',
                   ont = "BP",
                   pAdjustMethod = "BH",
...
ego_BP_down <- enrichGO(gene = downDEGs$external_gene_name.y,
                   OrgDb = org.Hs.eg.db,
                   keyType = 'SYMBOL',
                   ont = "BP",
                   pAdjustMethod = "BH",
...
ego_MF_up <- enrichGO(gene = upDEGs$external_gene_name.y,
                   OrgDb = org.Hs.eg.db,
                   keyType = 'SYMBOL',
                   ont = "MF",
                   pAdjustMethod = "BH",
...
ego_MF_down <- enrichGO(gene = downDEGs$external_gene_name.y,
                   OrgDb = org.Hs.eg.db,
                   keyType = 'SYMBOL',
                   ont = "MF",
                   pAdjustMethod = "BH",
...
ecdf = motifEcdf(PWM, organism = "hg19", quick = TRUE) # calculate for each motif of my gene the empirical distribution scores
# ecdf
...
fractions_99_75 = c()
fractions_99_75 = c(fractions_99_75, length(which(apply(scores, 1, sum)>0))/100)
fractions_99_75
```
