...
# Analysis
```{r load libraries, results='hide', include=FALSE}
library(tidyverse)
library(biomaRt)
library(MotifDb)
library(seqLogo) 
...
filter_vec <- apply(raw_counts_df_protein_coding,1, function(y) max(by(y, c_anno_df$condition, function(x) sum(x>=count_thr)))) # groups the values on each condition and sum all the values above the count
```
...
write(genes_id$ensembl_gene_id, 'output/enriched_genes_id.txt')
```
...
plot(net.c, 
     ...
     edge.curved=0.1)
# this first plot looks very confusing, there are a lot of connections. Let's try to take only nodes with at least a certain degree value.
