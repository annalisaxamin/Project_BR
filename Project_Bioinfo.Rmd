...
genes_id # down_regulated genes that has a region in their promoter with binding scores above the defined threshold for any PWMs
...
```{r}
dat <- read.table("down_DEGs.txt", sep = "\t", header = TRUE)

write.table(dat$ensembl_gene_id, file = "ens_gene_id_down.txt", row.names = FALSE, col.names = FALSE)
```
...
downDEGs <- read.table("down_DEGs.txt", sep = "\t", header = TRUE)
